...
def UpdatePassword():
    if request.method=='POST':
        ...
        print(usr.password, generate_password_hash(new_pass, method='sha256'))
        new_pass_hash = generate_password_hash(new_pass, method='sha256')
        if check_password_hash(usr.password, old_pass):
            usr.password=generate_password_hash(new_pass, method='sha256')
            db.session.commit()
...
def new1():
    if request.method == 'POST':
        ...
        new_user1 = User(fname=fname, lname=lname, email=email, phone=phone,dob=dob,password=generate_password_hash(password, method='sha256'),weight=weight,height=height,age=age,gender=gender,bodytype=bodytype,activity=activity,goal=goal,health_issues1=health_issues1,health_issues2=health_issues2,allergy1=allergy1,allergy2=allergy2,cal=cal_, fat=fat_, protein=protein_, carbs=carbs_)
        if new_user1:
...
def signup():
    ...
    if request.method == 'POST':
        ...
        # create a new user with the form data. Hash the password so the plaintext version isn't saved.
            new_user = User(fname=fname, lname=lname, email=email, phone=phone,dob=dob,password=generate_password_hash(password, method='sha256'),weight=weight,height=height,age=age,gender=gender,bodytype=bodytype,activity=activity,goal=goal,health_issues1=health_issues1,health_issues2=health_issues2,allergy1=allergy1,allergy2=allergy2,cal=cal_, fat=fat_, protein=protein_, carbs=carbs_)
            if new_user:
